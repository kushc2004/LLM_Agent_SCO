...
class Evaluator(Agent):
    ...
    def take_input(self, p):
        ...
        if len(p["shape"]) == 0:
            ...
            input_method = input("Do you want to input the data manually or from an Excel file? (manual/excel): ").strip().lower()
            ...
            if input_method == "manual":
                ...
                while t > 0:
                    try:
                        ...
                        if isinstance(result_list, list):
                            ...
                            print("The input is not a valid list. Try again...")
                            input_string = input("Enter Value in Matrix(List) form: ")
                    except (ValueError, SyntaxError):
                        print("Invalid input format. Try again...")
                        input_string = input("Enter Value in Matrix(List) form: ")
            elif input_method == "excel":
                file_path = input("Enter the file path of the Excel file: ").strip()
                try:
                ...
                print("Invalid input method. Please choose 'manual' or 'excel'.")
                return None
