...
5. Identify all the given numerical data in the prompt,
8. Use '```json' and '```' to enclose the json file.
- Go through the statements one by one, and identify and separate all the parameters and variables of the problem, you should not miss any parameter or vairable, and put them in a json file and a string in this format:
...
{{
    ...
    "data": [
        {{
            "<parameter_name>": "int or float or matrix or array"
        }}
    ],
    "path": "{folder_dir}"
}}
...
- "symbol" is the mathematical symbol you want to use to represent the parameter or variable. **STRICT INSTRUCTION** All symbols should be unique (without the indices), no symbol should be repeated. Example: {{Distance}}_{{ij}} and {{Distance}}_jk are different symbols but if checked withoit indices then they are same. Such representation is wrong. Use {{Distanceij}}_{{ij}} or {{Distancejk}}__jk something like this. Even if you are defining variables then each of their symbols should be different as well. 
...
- "data" is the numerical values of the data given in the problem description. Express it in the matrix or array form which you can decide by checking the shape of the parameter. Include the data of all the parameters or variables mentioned in the problem. Ensure the data of all the parameters is present here. Do not miss any single parameter.
...
- Before defining every shape check all the parameter symbols and then only define the shape. Ensure that whatever you are writing in the shape should be a symbol of a parameter.
...
- If there is a new line in description then instead of typing new line include a new line (double slash n) \\\\n character as the json file does not recognize a new line. 
- Not inlcude a tab sequence while generating the updated description. Use four spaces instead.
- **Strict Instruction** : Note that I'm going to use python json.loads() function to parse the json file, so please make sure the format is correct (don't add ',' before enclosing '}}' or ']' characters). We should not get any invalid control character error.
...
def transform_problem(folder_dir: str):
    ...
    prompt = prompt_template.format(description=description, folder_dir=folder_dir)
